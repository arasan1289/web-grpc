...
echoapp.EchoApp.prototype.downloadFile = function(fileName) {
  var streamRequest = new this.ctors.ServerDownloadRequest();
  streamRequest.setFileName(fileName);
  

  var stream = this.echoService.download(
    streamRequest,
    {"custom-header-1": "value1"});
  var self = this;
  var fileName;
  var responseArrayBuffer=[]
  stream.on('data', function(response) {
    fileName=response.getFileName()
    console.log(response.getFileChunk())
    responseArrayBuffer.push(response.getFileChunk())
    // echoapp.EchoApp.addRightMessage(response.getFileChunk());
  });
  stream.on('status', function(status) {
    self.handlers.checkGrpcStatusCode(status);
    if (status.metadata) {
  ...
  stream.on('end', function() {
    var blob=new Blob(responseArrayBuffer,{type: "application/octet-stream" })
...
echoapp.EchoApp.prototype.download = function(e) {
  var fileName = $("#text").val().trim();
...
echoapp.EchoApp.prototype.load = function() {
  ...
  $(document).ready(function() {
    ...
    $("#download").click(self.download.bind(self));
